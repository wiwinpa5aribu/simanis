...
export function errorHandler(
  ...
  if (error instanceof UnauthorizedError) {
    ...
    return reply
      .status(401)
      .send(createErrorResponse('UNAUTHORIZED', error.message))
  }
  ...
  if (error instanceof ForbiddenError) {
    ...
    return reply
      .status(403)
      .send(createErrorResponse('FORBIDDEN', error.message))
  }
  ...
  if (error instanceof ConflictError) {
    ...
    return reply
      .status(409)
      .send(createErrorResponse('CONFLICT', error.message))
  }
  ...
  return reply
    .status(500)
    .send(
      createErrorResponse('INTERNAL_ERROR', 'Terjadi kesalahan pada server')
    )
}
