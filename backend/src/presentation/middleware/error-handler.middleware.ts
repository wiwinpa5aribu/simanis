...
export function errorHandler(error: FastifyError, request: FastifyRequest, reply: FastifyReply) {
    ...
    if (error instanceof ValidationError) {
        ...
        return reply.status(400).send(createErrorResponse('VALIDATION_ERROR', error.message, error.details));
    }
    ...
    return reply.status(500).send(createErrorResponse('INTERNAL_ERROR', 'Terjadi kesalahan pada server'));
}
