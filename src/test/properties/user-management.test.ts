...
function generateMockUser(): User {
  ...
  return {
    ...
    email: Math.random() > 0.3 ? `user${Math.floor(Math.random() * 1000)}@test.com` : null,
    roles: userRoles,
...
describe('Property 6: User Search Filter Correctness', () => {
  it('should filter users by name containing search term (case-insensitive)', () => {
    ...
    for (const term of searchTerms) {
      ...
      for (const user of filtered) {
        ...
        const usernameMatch = user.username.toLowerCase().includes(term.toLowerCase())
  ...
  it('should be case-insensitive', () => {
    const users: User[] = [
      { id: 1, name: 'John Doe', username: 'johndoe', email: null, roles: ['Admin'], createdAt: new Date().toISOString() },
      { id: 2, name: 'Jane Smith', username: 'janesmith', email: null, roles: ['Operator'], createdAt: new Date().toISOString() },
    ]
