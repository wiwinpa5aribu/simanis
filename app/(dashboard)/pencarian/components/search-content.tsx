...
const quickFilters = [
  ...
  { label: "Aset Tidak Aktif", status: "tidak_aktif", condition: "" },
]
...
export function SearchContent({ initialAssets, locations }: SearchContentProps) {
  ...
  const handleQuickFilter = (filter: (typeof quickFilters)[0]) => {
    ...
    setConditionFilter(filter.condition || "all")
    setShowResults(true)
  }
  ...
  const getStatusBadge = (status: TAsset["status"]) => {
    ...
    return <Badge variant={variants[status] || "default"}>{config?.label || status}</Badge>
  }
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-foreground">
                Hasil Pencarian ({filteredAssets.length} aset)
              </CardTitle>
              <div className="flex gap-2">
