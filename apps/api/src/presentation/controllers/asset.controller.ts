...
export class AssetController {
  ...
  static async getAll(request: FastifyRequest, reply: FastifyReply) {
    ...
    return reply
      .status(200)
      .send(createSuccessResponse(result.assets, result.meta))
  }
  ...
  static async delete(
    ...
    const deleteAssetUseCase = new DeleteAssetUseCase(
      assetRepository,
      auditRepository
    )
    ...
    return reply
      .status(200)
      .send(createSuccessResponse({ message: 'Aset berhasil dihapus' }))
  }
  ...
  static async createMutation(
    ...
    const { id } = request.params
    const { toRoomId, note } = request.body as {
      toRoomId: number
      note?: string
    }
