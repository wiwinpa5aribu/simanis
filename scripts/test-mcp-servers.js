...
for (const [name, config] of Object.entries(servers)) {
  ...
  const autoApprove = config.autoApprove?.length > 0 
...
console.log(`   Enabled: ${enabledServers.length} (${enabledServers.join(', ')})`);
console.log(`   Disabled: ${disabledServers.length} (${disabledServers.join(', ')})`);
